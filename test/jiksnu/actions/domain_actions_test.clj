...
(test-environment-fixture
     ...
     (let [domain (model.domain/fetch-by-id (:_id domain))]
       (context "when there is no url context"
         (let [url (factory/make-uri domain-name "/1")
           ...
           (actions.domain/fetch-xrd domain url) => (contains {:_id domain-name})
       ...
       (let [url       (format "http://%s/status/users/1"                      domain-name)
         ...
         (context "and the bare domain does not have a host meta"
           ...
           (context "and none of the subpaths have host metas"
             (actions.domain/fetch-xrd domain url) => nil
             (provided
 ...
 ;; path first
 #_(context #'actions.domain/discover-webfinger
   (let [domain (mock/a-domain-exists)
         domain-name (:_id domain)]

     #_(context "when the domain doesn't have a web interface"
